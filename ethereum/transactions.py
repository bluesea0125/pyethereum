...
def main():
    key = utils.sha3(b'dog')
    addr = utils.privtoaddr(key)
    key2 = utils.sha3(b'cat')
    addr2 = utils.privtoaddr(key2)

    tx = Transaction(
        nonce=0, gasprice=0, startgas=100000,
        to=addr2, value=utils.denoms.finney * 10, data=b'').sign(key)
